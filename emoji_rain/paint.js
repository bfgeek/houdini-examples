...
function giveMeARandomEmoji(width, height) {
  var emoji = {}
  emoji.code = _emoji[Math.floor((Math.random() * _totalEmoji))];
  emoji.char = _fromCodePoint(emoji.code);

  // 1 to window size
  emoji.x = Math.floor((Math.random() * width) + 1);
  emoji.y = Math.floor((Math.random() * height) + 1);
